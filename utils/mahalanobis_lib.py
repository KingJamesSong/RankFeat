...
def sample_estimator(model, num_classes, feature_list, train_loader):
    ...
    for num_batch, (data, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            data = Variable(data)
            data = data.cuda()
            output, out_features = model.intermediate_forward(x=data, layer_index='all')
...
def get_Mahalanobis_score(inputs, model, num_classes, sample_mean, precision, num_output, magnitude):
    ...
    for layer_index in range(num_output):
        ...
        out_features = model.intermediate_forward(x=data, layer_index=layer_index)
        ...
        noise_out_features = model.intermediate_forward(x=Variable(tempInputs), layer_index=layer_index)
